...
static int magic_type_check_func(void)
{
	int ret;

	int security_wp_magic = 0;
	int security_wp_type = 0;

	ret = secure_wp_magic_read(&security_wp_magic, &security_wp_type);
	if (ret){
		pr_err("[OTR]%s: secure_wp_magic_read error\n", __func__);
		return -1;
	}else{
		pr_info("[OTR]%s: magic: %d(%x) type: %d(%x)\n", __func__,
			security_wp_magic, security_wp_magic, security_wp_type, security_wp_type);
		if(security_wp_magic == 0x4D4C1717 && security_wp_type == 0x9 ){
			pr_err("[OTR]%s: magic & type is One Time Root\n", __func__);
			return 1;
		}else
			return 0;
	}
...
static enum pon_power_off_type htc_restart_cmd_to_type(const char* cmd)
{
	...
	if(is_force_soft==1){
		pr_err("[OTR]%s: PON_POWER_OFF_WARM_RESET\n", __func__);
		return PON_POWER_OFF_WARM_RESET;
	}
...
static void msm_restart_prepare(char mode, const char *cmd)
{
	...
	if (in_panic) {
		...
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			set_restart_action(RESTART_REASON_RAMDUMP, "force-hard");
		} else {
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	is_force_soft = magic_type_check_func();
