			     ...
			     int op_enable)
{
	...
	default:
		...
		if (mfd->panel_info->cont_splash_enabled && mfd->panel_info->first_power_on)
			break;
	...
	struct sync_fence **fences, int fence_cnt)
{
	...
	for (i = 0; i < fence_cnt && !ret; i++) {
		...
		if (ret == -ETIME) {
			...
			for (j=0;j<5;j++)
			{
				snesor.sensor_num = (uint32_t)j;
				tsens_get_temp(&snesor,&temp);
				temperature[j] = (int)temp;
				printk("[MDP] Sensor %d = %d",j,(int)temp);
			}

			if (temperature[0] >= HIGH_TEMP && temperature[1] >= HIGH_TEMP && temperature[2] >= HIGH_TEMP && temperature[3] >= HIGH_TEMP && temperature[4] >= HIGH_TEMP)
				panic("MDP Fence timeout with high temperature");
