							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
		...
		break;
	case BL_I2C:
		bl_level = (unsigned char)shrink_pwm(bl_level, ctrl_pdata->pwm_min, ctrl_pdata->pwm_default, ctrl_pdata->pwm_max);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (vzw_sku) {
		...
		ctrl_pdata->pwm_min = 6;
		ctrl_pdata->pwm_default = 63;
		ctrl_pdata->pwm_max = 255;
		pinfo->camera_blk = 197;
	}
#endif
...
int __init cmdline_board_cid_check(char *s)
{
	vzw_sku = !strncmp(s, "VZW__001", 8);
	return 1;
}
__setup("androidboot.cid=", cmdline_board_cid_check);
