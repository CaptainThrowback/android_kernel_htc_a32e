...
#define MSM_TZLOG_PHYS		0x8E3D0000
#endif
...
static int _disp_tz_htc_log_stats(char __user *ubuf, size_t count, loff_t *offp)
{
    ...
    printk("%s, %d, tz_scm_log_phys=0x%08X\n", __func__, __LINE__, htc_tzlog->tz_scm_log_phys);
    printk("%s, %d, count=%d\n", __func__, __LINE__, count);

    printk("%s, %d, pr_cursor=%p, pw_cursor=%p\n", __func__, __LINE__, pr_cursor, pw_cursor);
	if (buf != 0 && (count < MSM_TZLOG_SIZE)) {
		
		r_cursor = readl_relaxed(pr_cursor);
		w_cursor = readl_relaxed(pw_cursor);
        printk("%s, %d, r_cursor=%d, w_cursor=%d\n", __func__, __LINE__, r_cursor, w_cursor);
...
static int tz_log_probe(struct platform_device *pdev)
{
	...
	pr_info("[TZLOG] %pR virt:%pf\n",
			resource, virt_iobase);
	...
	pr_info("[TZLOG] buffer address:%x size:%x virt:%pf\n",
		tz_scm_log_phys, tz_scm_log_size, htc_tzlog->buffer);
