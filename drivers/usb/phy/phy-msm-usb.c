...
#define XD	3
#define PVT	0x80
extern unsigned int usb_project_pid;
static void ulpi_init(struct msm_otg *motg)
{
	...
	if (usb_project_pid == 0x066B || usb_project_pid == 0x0662)
		seq[2] = 0x17;
...
phcd_retry:
	...
	printk(KERN_WARNING "[USB] USB in low power mode\n");
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	printk(KERN_WARNING "setting usb power supply type %d\n", charger_type);
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	printk(KERN_WARNING "[USB] %s begining work\n", usb_otg_state_string(otg->phy->state));
	...
	pr_info("%s work, motg->inputs=[0x%lx], otg->host=[0x%x]\n", usb_otg_state_string(otg->phy->state), motg->inputs, (unsigned int)otg->host);
...
void msm_otg_set_vbus_state(int online)
{
	...
	printk(KERN_WARNING "[USB] %s:%d\n", __func__, online);
	if (online) {
		printk(KERN_WARNING "PMIC: BSV set\n");
	...
	if (!init) {
		...
		printk(KERN_WARNING "PMIC: BSV init complete\n");
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	printk(KERN_WARNING "ID status_w\n");
...
{
	struct msm_otg *motg = container_of(psy, struct msm_otg, usb_psy);
	printk(KERN_WARNING "[USB] otg_power_set_property_usb: %d\n",psp);
...
msm_otg_ext_chg_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSM_USB_EXT_CHG_BLOCK_LPM:
		if (get_user(val, (int __user *)arg)) {
			printk(KERN_WARNING "%s: get_user failed\n\n", __func__);
			ret = -EFAULT;
			break;
		}
...
static int msm_otg_pinctrl_control(struct platform_device *pdev, struct msm_otg *motg)
{
	int ret = 0;
	...
	if (IS_ERR(motg->phy_pinctrl)) {
		if (of_property_read_bool(pdev->dev.of_node, "pinctrl-names")) {
			printk(KERN_INFO "Error encountered while getting pinctrl");
	...
	if (ret) {
		printk(KERN_WARNING "[USB] %s: can't init GPIO!\n",__func__);
		return ret;
	}
	motg->adc_sel_init = pinctrl_lookup_state(motg->phy_pinctrl, "usb_adc_sel_init");
	if (IS_ERR_OR_NULL(motg->adc_sel_init)){
		printk(KERN_WARNING "[USB] %s: can't get adc sel pinctrl state\n", __func__);
		ret = PTR_ERR(motg->gpio_state_init);
		motg->phy_pinctrl = NULL;
		return ret;
	}
	ret = pinctrl_select_state(motg->phy_pinctrl, motg->adc_sel_init);
	if (IS_ERR_OR_NULL(motg->adc_sel_init)){
		printk(KERN_WARNING "[USB] %s: can't init ADC SEL gpio\n", __func__);
		ret = PTR_ERR(motg->gpio_state_init);
		motg->phy_pinctrl = NULL;
		return ret;
	}
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	printk(KERN_WARNING "[USB] msm_otg probe\n");
	...
	printk(KERN_WARNING "OTG regs = %p\n", motg->regs);
	...
	ret = msm_otg_pinctrl_control(pdev, motg);
	if (ret) {
		printk(KERN_WARNING "%s: pinctrl error\n", __func__);
		goto free_ldo_init;
	}
	...
	if (1) {
		...
		psy = power_supply_get_by_name("usb");
		dev_info(motg->phy.dev, "[USB] psy=[0x%x]\n", (unsigned int)psy);
...
int64_t  htc_qpnp_adc_get_usbid_adc(void)
{
	...
	err = qpnp_vadc_read(motg->vadc_chip ,17 , &result);
...
EXPORT_SYMBOL(htc_qpnp_adc_get_usbid_adc);
