			  ...
			  struct sk_buff_head *list)
{
	...
	if (get_unaligned_le32(tmp) != opts->nth_sign) {
		DBG(port->func.config->cdev, "Wrong NTH SIGN, skblen %d\n",
			skb->len);
	...
	if (get_unaligned_le16(tmp++) != opts->nth_size) {
		DBG(port->func.config->cdev, "Wrong NTB headersize\n");
		goto err;
	}
	tmp++; 
	...
	if (get_ncm(&tmp, opts->block_length) > max_size) {
		DBG(port->func.config->cdev, "OUT size exceeded\n");
		goto err;
	}
	...
	if (((index % 4) != 0) && (index < opts->nth_size)) {
		DBG(port->func.config->cdev, "Bad index: %x\n",
			index);
		goto err;
	}
	...
	if ((ndp_len < opts->ndp_size + 2 * 2 * (opts->dgram_item_len * 2))
		...
		DBG(port->func.config->cdev, "Bad NDP length: %x\n", ndp_len);
		goto err;
	}
	tmp += opts->reserved1;
	tmp += opts->next_fp_index; 
	...
	do {
		...
		if (dg_len < 14 + crc_len) { 
			DBG(port->func.config->cdev, "Bad dgram length: %x\n",
			     dg_len);
			goto err;
		}
		if (ncm->is_crc) {
			...
			crc2 = ~crc32_le(~0,
					 ...
					 dg_len - crc_len);
			if (crc != crc2) {
				DBG(port->func.config->cdev, "Bad CRC\n");
				goto err;
			}
		}
