...
unsigned int check_one_time_root_flag(void);
...
static DEFINE_MUTEX(android_enable_sem);
...
struct android_dev {
	...
	struct usb_function *usb_serial_function_pool[MAX_SERIAL_INSTANCES];
	enum fserial_func_type serial_func_type[MAX_SERIAL_INSTANCES];
	int autobot_mode;
	atomic_t adb_ready;
...
#define PM_QOS_LATENCY_PERF		2
#define PM_QOS_LATENCY_NORMAL		1001
#define PM_QOS_LATENCY_SLEEP		12701
...
static void android_work(struct work_struct *data)
{
	...
	unsigned long flags;
	int pm_qos_vote = -1;
	printk(KERN_INFO "[USB] android_work : sw_suspended %d suspended %d config %d,connect2pc %d\n",dev->sw_suspended,dev->suspended,cdev->config?1:0,connect2pc);
	...
	if (connect2pc != dev->sw_connected) {
		...
		printk(KERN_WARNING "set usb_connect2pc = %d\n", connect2pc);
		if (!connect2pc) {
...
static int android_enable(struct android_dev *dev)
{
	...
	struct android_configuration *conf;
	int err = 0;
	mutex_lock(&android_enable_sem);

	printk(KERN_WARNING "[USB] %s depth %d\n",__func__,dev->disable_depth);

	if (WARN_ON(!dev->disable_depth)) {
		mutex_unlock(&android_enable_sem);
		return err;
	}
...
static ssize_t func_en_show(struct device *pdev, struct device_attribute *attr,char *buf)
{
...
static ssize_t func_en_store(struct device *pdev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	...
	int value;
	sscanf(buf, "%d", &value);
	if (!strcmp(func->name, "ffs"))
		snprintf(state_buf, sizeof(state_buf), "SWITCH_STATE=%s", "adb");
	else
		snprintf(state_buf, sizeof(state_buf), "SWITCH_STATE=%s", func->name);
...
static DEVICE_ATTR(func_en, S_IRUGO | S_IWUSR | S_IWGRP, func_en_show, func_en_store);
					...
					const char *buf, size_t size)
{
	...
	dev = list_first_entry(&android_dev_list, struct android_dev,
					list_item);
	mutex_lock(&dev->mutex);
	printk(KERN_WARNING "[USB] %s ffs_aliases %s\n",__func__,buf);
...
#define FFS_ENABLED_TIMEOUT_MS 100
	if (0 == wait_event_interruptible_timeout(dev->wait_q,
				config->enabled, msecs_to_jiffies(FFS_ENABLED_TIMEOUT_MS))) {
		pr_info("wait for ffs enable ready timeout (%d) ms", FFS_ENABLED_TIMEOUT_MS);
	}
					 ...
					 struct usb_configuration *c)
{
	...
	if (!rmnet_initialized) {
		...
		if (dev->pdata->rmnet_transports_interface)
			strcpy(rmnet_transports, dev->pdata->rmnet_transports_interface);
		else
			strcpy(rmnet_transports, "qti,bam");
					...
					struct usb_configuration *c)
{
	...
	if (dev->pdata->diag_client_interface)
		strcpy(diag_clients, dev->pdata->diag_client_interface);
	else
		strcpy(diag_clients, "diag");
					...
					struct usb_composite_dev *cdev)
{
	...
	if (check_tracfone_flag()) {
		if (check_one_time_root_flag())
		{
			printk(KERN_INFO "%s: use serial init string on TF sku.\n", __func__);
		} else {
			strcpy(serial_transports, "tty,tty,tty,tty,tty:acm");
		}
	}

	strncpy(buf, serial_transports, sizeof(buf));
	buf[63] = 0;
	printk(KERN_WARNING "[USB] serial_function_init string %s\n",buf);
	b = strim(buf);
	while (b) {
		str[0] = str[1] = 0;
		name = strsep(&b, ",");
		...
		printk(KERN_WARNING "[USB] serial_function_init loop %d %s %s %s\n",serial_nports,b,str[0],str[1]);
		err = gserial_init_port(serial_nports, str[0], str[1]);
		if (err) {
			pr_err("serial: Cannot open port '%s'\n", str[0]);
			return -ENXIO;
		}
		_android_dev->serial_func_type[serial_nports]  = serial_str_to_func_type(str[1]);
		serial_nports++;
	}
	err = gport_setup(cdev->config);
	if (err) {
		pr_err("serial: Cannot setup transports err : %d",err);
		return -ENXIO;
	}
	for (i = 0; i < serial_nports; i++) {
		...
		if (IS_ERR(config->f_serial_inst[i])) {
			...
			printk(KERN_WARNING "[USB] fail to get usb function instance ger\n");
			return -ENXIO;
		}
		config->f_serial[i] = usb_get_function(config->f_serial_inst[i]);
		_android_dev->usb_serial_function_pool[i] = config->f_serial[i];
		if (IS_ERR(config->f_serial[i])) {
			err = PTR_ERR(config->f_serial[i]);
			printk(KERN_WARNING "[USB] fail to get usb function ger\n");
			return -ENXIO;
		}
	}
#if 0
...
{
	int err = -1, i;
	int car_mode = _android_dev->autobot_mode;
#if 0
	...
	for (i = 0; i < serial_nports; i++) {
		if ((f->android_dev->serial_func_type[i] == USB_FSER_FUNC_SERIAL) ||
				(car_mode && f->android_dev->serial_func_type[i] == USB_FSER_FUNC_AUTOBOT))
		{
			err = usb_add_function(c, _android_dev->usb_serial_function_pool[i]);
			if (err) {
...
err_gser_usb_add_function:
	while (i-- > 0){
		if ((f->android_dev->serial_func_type[i] == USB_FSER_FUNC_SERIAL) ||
				(car_mode && f->android_dev->serial_func_type[i] == USB_FSER_FUNC_AUTOBOT))
			usb_remove_function(c, _android_dev->usb_serial_function_pool[i]);
	}
		...
		struct usb_configuration *c)
{
	...
	for (i = 0; i < serial_nports; i++) {
		if (f->android_dev->serial_func_type[i] == USB_FSER_FUNC_MODEM) {
			err = usb_add_function(c,_android_dev->usb_serial_function_pool[i]);
			if (err) {
				pr_err("Could not bind gser%u config\n", i);
				return -1;
...
static unsigned int disk_mode_enable;
					...
					struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata) {
		...
		config->fsg.nluns = dev->pdata ? dev->pdata->nluns : 1;
		if (disk_mode_enable)
			config->fsg.nluns++;
		for (i = 0; i < config->fsg.nluns; i++) {
			if (dev->pdata->cdrom_lun && i == (config->fsg.nluns-1)) {
			...
			snprintf(name[i], MAX_LUN_NAME, "lun%d", i);
			pr_info("%s: lun%d attr: ro:%d cdrom:%d\n", __func__, i, config->fsg.luns[i].ro, config->fsg.luns[i].cdrom);
		}
	} else {
		config->fsg.nluns = 1;
...
static struct android_usb_function *supported_functions_for_tracfone[] = {
	&ffs_function,
	&mbim_function,
#ifdef CONFIG_SND_PCM
	&audio_function,
...
static struct android_usb_function *supported_functions_for_tracfone_with_lab[] = {
	&ffs_function,
	&mbim_function,
#ifdef CONFIG_SND_PCM
	&audio_function,
				  ...
				  struct usb_composite_dev *cdev)
{
	...
	for (; (f = *functions++); index++) {
		...
		if (device_create_file(f->dev, &dev_attr_func_en) < 0) {
			    ...
			    const char *buff, size_t size)
{
	...
	printk(KERN_WARNING "[USB] soft disconnect %d\n",enabled);
...
static void htc_and_mute_disconnect(struct usb_gadget *gadget){
	composite_mute_disconnect(gadget);
}

static void htc_and_disconnect(struct usb_gadget *gadget)
{
	struct usb_composite_dev *cdev = get_gadget_data(gadget);
	struct android_dev *dev = cdev_to_android_dev(cdev);
	unsigned long flags;
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		printk(KERN_WARNING "[USB] %s: init by pdata.\n", __func__);
		pdata = pdev->dev.platform_data;
		strlcpy(pdata->streaming_func[0], "rndis", sizeof(pdata->streaming_func[0]));
		strlcpy(pdata->streaming_func[1], "mtp", sizeof(pdata->streaming_func[1]));
		strlcpy(pdata->streaming_func[2], "cdc_network", sizeof(pdata->streaming_func[2]));
		pdata->streaming_func_count = 3;
	}
	...
	if (check_tracfone_flag()) {
		if (check_one_time_root_flag())
		{
			android_dev->functions = supported_functions_for_tracfone_with_lab;
		} else {
			android_dev->functions = supported_functions_for_tracfone;
		}
	} else
	...
	init_waitqueue_head(&android_dev->wait_q);
	...
	pr_info("%s(): registering android_usb_driver with core id:%d\n",
	...
	if (pdata) {
		pdata->pm_qos_latency[0] = PM_QOS_LATENCY_PERF;
		pdata->pm_qos_latency[1] = PM_QOS_LATENCY_NORMAL;
		pdata->pm_qos_latency[2] = PM_QOS_LATENCY_SLEEP;
...
static int __init init(void)
{
	...
	android_usb_driver.gadget_driver.disconnect = htc_and_disconnect;
	android_usb_driver.gadget_driver.mute_disconnect = htc_and_mute_disconnect;
