...
static unsigned int one_time_root_flag;
...
unsigned int check_one_time_root_flag(void)
{
	if (strcmp(htc_get_bootmode(),"ftm") == 0)
		...
		return one_time_root_flag;
}

static int __init get_one_time_root_flag(char *str)
{
	int ret = kstrtouint(str, 0, &one_time_root_flag);
	pr_info("one time root flag %d: %08x from %26s\r\n",
			ret, one_time_root_flag, str);
	return ret;
} early_param("ro.op", get_one_time_root_flag);
