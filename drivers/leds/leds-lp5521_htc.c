...
static uint32_t charging_flag = 0; 
...
static ssize_t lp5521_charging_led_switch_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", charging_flag);
}

static ssize_t lp5521_charging_led_switch_store(struct device *dev,
...
static DEVICE_ATTR(charging_led_switch, 0640, lp5521_charging_led_switch_show,
		lp5521_charging_led_switch_store);
...
static int lp5521_parse_dt(struct device *dev, struct led_i2c_platform_data *pdata)
{
	...
	prop = of_find_property(dt, "lp5521,charging_en", NULL);
	if (prop) {
		pdata->charging_gpio = of_get_named_gpio(dt, "lp5521,charging_en", 0);
	}
