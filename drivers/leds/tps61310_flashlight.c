...
int tps61310_torch_mode2(int mode2, int mode13)
{
	...
	if (mode2 == 0 && mode13 == 0)
		...
		current_hex = (( mode13 / 25 ) << 3 ) + ( mode2 / 25 );
...
static int tps61310_pinctrl(struct tps61310_data *tps61310)
{
	int rc = 0;
	...
	tps61310->pinctrl = devm_pinctrl_get(&this_client->dev);
	if (IS_ERR_OR_NULL(tps61310->pinctrl)) {
		FLT_ERR_LOG("%s: Does not use pinctrl\n", __func__);
		tps61310->pinctrl = NULL;
		return 0;
	}
	tps61310->gpio_state_init = pinctrl_lookup_state(tps61310->pinctrl, "tps61310_active");
	if (IS_ERR_OR_NULL(tps61310->gpio_state_init)) {
		FLT_ERR_LOG("%s: Cannot get pintctrl state\n", __func__);
		rc = PTR_ERR(tps61310->gpio_state_init);
		tps61310->pinctrl = NULL;
		tps61310->gpio_state_init = NULL;
		return rc;
	}
	rc = pinctrl_select_state(tps61310->pinctrl, tps61310->gpio_state_init);
	if (rc) {
		FLT_ERR_LOG("%s: Cannot init INT gpio\n", __func__);
		return rc;
	}
