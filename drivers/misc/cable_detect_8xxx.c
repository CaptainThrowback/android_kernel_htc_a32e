...
int __maybe_unused cable_detect_register_notifier(struct t_cable_status_notifier *notifier)
{
...
static void cable_detect_handler(struct work_struct *w)
{
	...
	CABLE_INFO("%s ID pin %d, type %d", __func__,
...
static void htc_config_usb_id_gpios(bool output)
{
	struct cable_detect_info *pInfo = &the_cable_info;

	if (output) {
		if (gpio_direction_output(pInfo->usb_id_pin_gpio, 1)) {
			printk(KERN_ERR "[CABLE] fail to config usb id, output = %d\n",output);
			return;
		}
		pr_info("[CABLE] %s: %d output high\n",  __func__, pInfo->usb_id_pin_gpio);
	} else {
		if (gpio_direction_input(pInfo->usb_id_pin_gpio)) {
			printk(KERN_ERR "[CABLE] fail to config usb id, output = %d\n",output);
			return;
		}
		pr_info("[CABLE] %s: %d input nopull\n",  __func__, pInfo->usb_id_pin_gpio);
	}
}
...
static int cable_detect_get_adc(void)
{
	struct cable_detect_info *pInfo = &the_cable_info;
	long val;
	gpio_set_value(pInfo->adc_switch_gpio, 1);
	msleep(20);
	while((val = pInfo->get_adc_cb()) == -EAGAIN)
	{
		printk(KERN_WARNING "[CABLE] adc function is not ready. try again\n");
		msleep(1000);
	}
	gpio_set_value(pInfo->adc_switch_gpio, 0);
	return val;
...
int cable_get_usb_id_level(void)
{
	...
	if (pInfo->usb_id_pin_gpio)
		...
		printk(KERN_WARNING "usb id is not defined\n");
		return 1;
	}
}
...
static void usb_id_detect_init(struct cable_detect_info *pInfo)
{
	...
	ret = gpio_request(pInfo->adc_switch_gpio, "ADC_SWITCH_GPIO");
	if (ret) {
		CABLE_ERR("%s: request id gpio failed\n", __func__);
		return;
	}
	...
	gpio_direction_output(pInfo->adc_switch_gpio, 0);
...
static DEVICE_ATTR(dmb_wakeup, S_IWUSR, NULL, dmb_wakeup_store);
...
static DEVICE_ATTR(unlock_audio_dock_lock, S_IWUSR, NULL, unlock_audio_dock_lock_store);
...
static int cable_detect_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		pInfo->adc_switch_gpio = pdata->adc_switch_gpio();
		...
		printk(KERN_WARNING "%s(%d):id_gpio=%d, detect_type=%d, usb_id_pin_type=%d\n",
			__func__, __LINE__, pInfo->usb_id_pin_gpio,
			pInfo->detect_type, usb_id_pin_type);
	...
	if (ret != 0)
		CABLE_ERR("dev_attr_USB_ID_status failed\n");
...
static struct t_usb_status_notifier __maybe_unused usb_status_notifier = {
	.name = "cable_detect",
