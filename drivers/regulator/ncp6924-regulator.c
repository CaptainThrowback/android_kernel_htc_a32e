...
#include <linux/i2c.h>
#include <linux/regulator/ncp6924-regulator.h>
...
static const u32 INVALID_MODE_BIT = 0xff;
static const u32 INVALID_MODE     = 0xff;
...
{
	struct ncp6924_vreg *vreg = rdev_get_drvdata(rdev);
	struct ncp6924_vreg *ncp6924_dcdc2_list = NULL;
	struct device *dev = vreg->dev;
	struct i2c_client *client = to_i2c_client(dev);
	struct ncp6924_regulator *reg = i2c_get_clientdata(client);
	uint8_t val = 0;
...
static int ncp6924_set_mode(struct ncp6924_vreg *vreg, u32 mode)
{
        struct device *dev = vreg->dev;
        uint8_t val = 0;
        int rc = 0;
	...
	mutex_lock(&vreg->mlock);
        rc = ncp6924_i2c_read(dev, vreg->enable_addr, &val);
	if (rc) {
		pr_err("%s: read vreg enable status failed\n", __func__);
		goto fail_set_mode;
	}
	if (mode)
		val |= (1 << vreg->mode_bit);
	else
		val = val & ~(1 << vreg->mode_bit);
        rc = ncp6924_i2c_write(dev, vreg->enable_addr, val);
	if (rc) {
		pr_err("%s: write vreg mode failed\n", __func__);
		goto fail_set_mode;
	}
fail_set_mode:
	mutex_unlock(&vreg->mlock);;
...
static int ncp6924_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for_each_child_of_node(node, child) {
		...
		ret = of_property_read_u32(child, "ncp,mode-bit",
                                &ncp6924_vreg->mode_bit);
		if (ret)
			ncp6924_vreg->mode_bit = INVALID_MODE_BIT;

		ret = of_property_read_u32(child, "ncp,mode",
                                &ncp6924_vreg->mode);
		if (ret)
			ncp6924_vreg->mode = INVALID_MODE;
		...
		if (ncp6924_vreg->regulator_type == VREG_TYPE_DCDC)
			ncp6924_vreg->rdesc.n_voltages 	= 217;
		else if (ncp6924_vreg->regulator_type == VREG_TYPE_LDO)
			ncp6924_vreg->rdesc.n_voltages 	= 47;

		INIT_LIST_HEAD(&ncp6924_vreg->reg_list);
		mutex_init(&ncp6924_vreg->mlock);
		reg_config.dev = dev;
		reg_config.init_data = init_data;
		reg_config.driver_data = ncp6924_vreg;
		reg_config.of_node = child;
		ncp6924_vreg->rdev = regulator_register(&ncp6924_vreg->rdesc, &reg_config);
	...
	for (vreg_idx = 0; vreg_idx < reg->total_vregs;) {
		ncp6924_vreg = &reg->ncp6924_vregs[vreg_idx++];
		if (ncp6924_vreg->mode_bit != INVALID_MODE_BIT &&
			ncp6924_vreg->mode != INVALID_MODE) {
			ncp6924_set_mode(ncp6924_vreg, ncp6924_vreg->mode);
		}
	}
