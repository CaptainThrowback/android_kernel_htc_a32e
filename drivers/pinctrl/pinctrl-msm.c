...
struct msm_pintype_info *msm_pt;

int msm_dump_sdc(struct seq_file *m, int curr_len, char *sdc_buffer)
{
	unsigned int i, len;
	int val;
	struct msm_sdc_dump_info data;
	char list_sdc[100];
	char *title_msg = "------------ MSM SDC -------------";

	if (m)
		seq_printf(m, "%s\n", title_msg);
	else {
		pr_info("%s\n", title_msg);
		curr_len += sprintf(sdc_buffer + curr_len,
		"%s\n", title_msg);
	}

	for(i=0 ; i < MSM_PINTYPE_SDC_REGS_MAX ; i++){
		len = 0;
		val = __msm_sdc_get_dump_info(msm_pt, i, &data);

		switch(i){
		case 0:
			len += sprintf(list_sdc + len,"SDC1_CLK  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 1:
			len += sprintf(list_sdc + len,"SDC1_CMD  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 2:
			len += sprintf(list_sdc + len,"SDC1_DATA [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 3:
			len += sprintf(list_sdc + len,"SDC1_RCLK            ");
			break;
		case 4:
			len += sprintf(list_sdc + len,"SDC2_CLK  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 5:
			len += sprintf(list_sdc + len,"SDC2_CMD  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 6:
			len += sprintf(list_sdc + len,"SDC2_DATA [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 7:
			len += sprintf(list_sdc + len,"SDC3_CLK  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 8:
			len += sprintf(list_sdc + len,"SDC3_CMD  [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		case 9:
			len += sprintf(list_sdc + len,"SDC3_DATA [DRV]%2dmA, ", 2*(data.hdrv+1));
			break;
		}

		switch (data.pull) {
		case 0x0:
			len += sprintf(list_sdc + len, "[PULL]NO");
			break;
		case 0x1:
			len += sprintf(list_sdc + len, "[PULL]PD");
			break;
		case 0x2:
			len += sprintf(list_sdc + len, "[PULL]KP");
			break;
		case 0x3:
			len += sprintf(list_sdc + len, "[PULL]PU");
			break;
		default:
			break;
		}
...
#if defined(CONFIG_HTC_POWER_DEBUG) && defined(CONFIG_PINCTRL_MSM_TLMM)
	for(i=0;;i++)
		if(strcmp(dd->msm_pintype[i].name,"sdc") == 0)
			break;

	msm_pt = &dd->msm_pintype[i];
