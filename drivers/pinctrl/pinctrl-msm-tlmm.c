...
int __msm_sdc_get_dump_info(struct msm_pintype_info *msm_pt, unsigned pin_no, struct msm_sdc_dump_info *data)
{
	unsigned int val;
	void __iomem *cfg_reg;
	void __iomem *reg_base = msm_pt->reg_base;
	const struct msm_pintype_data *sdc_info = msm_pt->pintype_data;
	s32 offset = sdc_info->sdc_reg_offsets[pin_no];
	...
	data->pull = ( val >> sdc_regs[pin_no].pull_shft ) & sdc_regs[pin_no].pull_mask;
	if(pin_no != 3)
		data->hdrv = ( val >> sdc_regs[pin_no].drv_shft ) & sdc_regs[pin_no].drv_mask;
	return 0;
}
