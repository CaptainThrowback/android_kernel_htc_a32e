...
extern void force_disable_PM8909_VREG_ID_L15(void);
#endif
...
{
	int ret = 0;
	I("%s is %s\n", __func__, nfc_nci_nxp_hw_model[gDevice_info.HW_model]);
	ret = snprintf(buf,MAX_BUFFER_SIZE*2, "%s", nfc_nci_nxp_hw_model[gDevice_info.HW_model]);
	return ret;
}
...
{
	int ret = 0;
	I("%s is %x\n", __func__, (int)gDevice_info.fwVersion);
	ret = snprintf(buf,MAX_BUFFER_SIZE*2, "%x", (int)gDevice_info.fwVersion);
	return ret;
}
...
{
	int ret = 0;
	I("%s is %x\n", __func__, gDevice_info.manufactor);
	ret = snprintf(buf,MAX_BUFFER_SIZE*2, "%x", gDevice_info.manufactor);
	return ret;
}
...
static int mfg_nfc_test(int code)
{
	...
	case 99:
		I("Turn off NFC_PVDD");
		pn544_off_mode_charging(pn_info->client);
		break;
	default:
		E("%s: case default\n", __func__);
		break;
...
#if defined(CONFIG_MACH_A32E)
	NFC_I2C_SCL = of_get_named_gpio_flags(dt, "nfc_i2c_scl",
				0, &pdata->firm_gpio_flags);
	NFC_I2C_SDA = of_get_named_gpio_flags(dt, "nfc_i2c_sda",
				0, &pdata->firm_gpio_flags);
#endif
...
#if defined(CONFIG_MACH_A32E)
void pn544_power_off_sequence(void)
{
	int ret;
	...
	mdelay(50);
	force_disable_PM8909_VREG_ID_L15();
}
static void pn544_work(struct work_struct *work)
{
        struct pn544_dev *pni = pn_info;
        char nci_read_header[2] = {0};
        uint8_t receiverBuffer[MAX_NFC_DATA_SIZE] ={0};
...
#endif
static int nfc_ven_pinctrl(struct pn544_dev *pni)
{
	int retval;
	struct i2c_client *client = pni->client;
	int ret;

	I("%s",__func__);
	pni->pinctrl = devm_pinctrl_get(&client->dev);
	if(IS_ERR_OR_NULL(pni->pinctrl)){
		E("[pmx_nfc error]%s: Target does not use pinctrl\n",__func__);
		retval = PTR_ERR(pni->pinctrl);
		pni->pinctrl = NULL;
		return retval;
	}
	pni->gpio_state_init = pinctrl_lookup_state(pni->pinctrl, "nfc_ven");
	if(IS_ERR_OR_NULL(pni->gpio_state_init)){
                E("[pmx_nfc error]%s: Cannot get pintctrl state\n",__func__);
                retval = PTR_ERR(pni->gpio_state_init);
                pni->pinctrl = NULL;
                return retval;
	}
	ret = pinctrl_select_state(pni->pinctrl, pni->gpio_state_init);
	if(ret){
		E("[pmx_nfc error]%s: Cannot init INT gpio\n",__func__);
		return ret;
	}
	I("%s Pinctrl success!!",__func__);
	return 0;
}

static int nfc_dl_mode_pinctrl(struct pn544_dev *pni)
{
	int retval;
	struct i2c_client *client = pni->client;
	int ret;
	...
	pni->pinctrl = devm_pinctrl_get(&client->dev);
	if(IS_ERR_OR_NULL(pni->pinctrl)){
		E("[nfc_ctrl error]%s: Target does not use pinctrl\n",__func__);
		retval = PTR_ERR(pni->pinctrl);
		pni->pinctrl = NULL;
		return retval;
	}
	pni->gpio_state_init = pinctrl_lookup_state(pni->pinctrl, "nfc_dl_mode");
	if(IS_ERR_OR_NULL(pni->gpio_state_init)){
		E("[nfc_ctrl error]%s: Cannot get pintctrl state\n",__func__);
		retval = PTR_ERR(pni->gpio_state_init);
		pni->pinctrl = NULL;
		return retval;
	}
	ret = pinctrl_select_state(pni->pinctrl, pni->gpio_state_init);
	if(ret){
		E("[nfc_ctrl error]%s: Cannot init INT gpio\n",__func__);
		return ret;
	}
	I("%s Pinctrl success!!",__func__);
	return 0;
		...
		const struct i2c_device_id *id)
{
	...
	struct pn544_dev *pni;
        ...
        uint8_t NXP_PROPRIETARY_ACT[] = {0x2F,0x02,0x00};
...
#if defined(CONFIG_MACH_A32E)
			pn544_hw_reset_control(1);
                        INIT_DELAYED_WORK(&nfc_work, pn544_work);
                        ...
                        mdelay(NFC_READ_DELAY);
                        if(pn544_TxData(NXP_PROPRIETARY_ACT,3)<0)
                                E("%s: NXP_PROPRIETARY_ACT fail",__func__);
                        mdelay(NFC_READ_DELAY);
...
#else
			pn544_off_mode_charging(client);
#endif
                        break;
