		...
		struct bma250acc *acc)
{
	...
	if(1) {
		comres = bma250_smbus_read_byte_block(client,
				BMA255_ACC_X_LSB__REG, data, 6);

		acc->x = BMA250_GET_BITSLICE(data[0], BMA255_ACC_X_LSB)
			|(BMA250_GET_BITSLICE(data[1],
					BMA255_ACC_X_MSB)<<BMA255_ACC_X_LSB__LEN);
		acc->x = acc->x << (sizeof(short)*8-(BMA255_ACC_X_LSB__LEN
					+ BMA255_ACC_X_MSB__LEN));
		acc->x = acc->x >> (sizeof(short)*8-(BMA255_ACC_X_LSB__LEN
					+ BMA255_ACC_X_MSB__LEN));
		acc->y = BMA250_GET_BITSLICE(data[2], BMA255_ACC_Y_LSB)
			| (BMA250_GET_BITSLICE(data[3],
					BMA255_ACC_Y_MSB)<<BMA255_ACC_Y_LSB__LEN);
		acc->y = acc->y << (sizeof(short)*8-(BMA255_ACC_Y_LSB__LEN
					+ BMA255_ACC_Y_MSB__LEN));
		acc->y = acc->y >> (sizeof(short)*8-(BMA255_ACC_Y_LSB__LEN
					+ BMA255_ACC_Y_MSB__LEN));

		acc->z = BMA250_GET_BITSLICE(data[4], BMA255_ACC_Z_LSB)
			| (BMA250_GET_BITSLICE(data[5],
					BMA255_ACC_Z_MSB)<<BMA255_ACC_Z_LSB__LEN);
		acc->z = acc->z << (sizeof(short)*8-(BMA255_ACC_Z_LSB__LEN
		...
		const struct i2c_device_id *id)
{
	...
	for (i = 0; i < 3; i++) {
		tempvalue = i2c_smbus_read_byte_data(client,
						     BMA250_CHIP_ID_REG);
		if ((tempvalue == BMA250_CHIP_ID) ||
		    (tempvalue == BMA250E_CHIP_ID)||
		    (tempvalue == BMA255_CHIP_ID)
		   ) {
