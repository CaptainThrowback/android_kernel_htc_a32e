...
static bool polling_limit;
static int preserved_polling_interval;
...
#define	ASENSE_TARGET_BMA255E			0x00B4		
#define	ASENSE_TARGET_BMA250E			0x02D0		
static int is_bma255e = 0;
#define TIME_LOG		50	
	...
	const char *buf, size_t count)
{
	...
	power_key_pressed = 0;
    } else if (powerkey_pressed == 2) {
        polling_limit = true;
        akm8975_misc_data->poll_interval = 200;
        I("%s: Limit to 200ms polling interval, preserved = %d\n", __func__,
          preserved_polling_interval);
    } else if (powerkey_pressed == 3) {
        polling_limit = false;
        I("%s: Restore polling interval = %d\n", __func__,
          preserved_polling_interval);
        akm8975_misc_data->poll_interval = preserved_polling_interval;
    }
    return count;
}
...
static int BMA_pinctrl_init(void)
{
        int retval;
        int ret;

        D("%s: BMA_pinctrl_init", __func__);
        
        gdata->pinctrl = devm_pinctrl_get(&this_client->dev);
        if (IS_ERR_OR_NULL(gdata->pinctrl)) {
                E("%s: Target does not use pinctrl\n", __func__);
                retval = PTR_ERR(gdata->pinctrl);
                gdata->pinctrl = NULL;
                return retval;
        }

        gdata->gpio_state_init = pinctrl_lookup_state(gdata->pinctrl, "bma255_acc_init");
        if (IS_ERR_OR_NULL(gdata->gpio_state_init)) {
                E("%s: Cannot get pintctrl state\n", __func__);
                retval = PTR_ERR(gdata->gpio_state_init);
                gdata->pinctrl = NULL;
                return retval;
        }

        ret = pinctrl_select_state(gdata->pinctrl, gdata->gpio_state_init);
        if (ret) {
                E("%s: Cannot init INT gpio\n", __func__);
                return ret;
        }
...
static int BMA_set_mode(unsigned char mode)
{
	...
	if (cir_flag && mode == bma250_MODE_SUSPEND) {
		I("%s: cir_flag = %d, do not set to bma250_MODE_SUSPEND\n",
		  __func__, cir_flag);
		return 0;
	} else {
#endif
	   ...
	   unsigned long arg)
{
	...
	case BMA_IOCTL_GET_CHIP_LAYOUT:
		if (pdata)
			temp = is_bma255e ? pdata->chip_layout : !(pdata->chip_layout);
		break;
	...
	if (atomic_read(&a_flag)) {
		...
		for (i = 0; i < 3; i++) {
			...
			gval[i] = ((buffer[i] + offset_buf[i] - user_offset[i])
					* ((is_bma255e) ? ASENSE_TARGET_BMA255E : ASENSE_TARGET_BMA250E)) / 256;
		}
		...
		g_acc[0] = ((pdata->negate_x)
			   ? (-gval[pdata->axis_map_x])
			   : (gval[pdata->axis_map_x]));
		g_acc[1] = ((pdata->negate_y)
			   ? (-gval[pdata->axis_map_y])
			   : (gval[pdata->axis_map_y]));
		g_acc[2] = ((pdata->negate_z)
			   ? (-gval[pdata->axis_map_z])
			   : (gval[pdata->axis_map_z]));
		...
		if (ignore_first_event >= 3) {
			...
			ignore_first_event++;
			DIF("Ignore X = %d Ignore Y = %d Ignore Z = %d\n ", g_acc[0], g_acc[1], g_acc[2]);
		}
...
void sensor_poll_interval_set(uint32_t poll_interval)
{
	...
	akm8975_misc_data->poll_interval = (polling_limit == true)
						? 200 : poll_interval;
	if (polling_limit == true) {
		D("%s: Limited to 200 ms\n", __func__);
	}
	      ...
	      unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_APP_SET_DELAY:
		...
		preserved_polling_interval = flag;
		I("%s: preserved_polling_interval = %d\n", __func__,
		  preserved_polling_interval);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	s += sprintf(s, "ChipID = 0x%X\n", buffer[0]);

	return s - buf;
}
static DEVICE_ACCESSORY_ATTR(ChipID, 0444, \
...
static int bma250_parse_dt(struct device *dev, struct bma250_platform_data *pdata)
{
	...
	if (prop) {
		...
		I("%s: g_sensor_bma250,chip_layout not found\n", __func__);
	...
	if (prop) {
		...
		I("%s: axis_map_x = %d\n", __func__, pdata->axis_map_x);
	} else
		I("%s: g_sensor_bma250,axis_map_x not found\n", __func__);
	...
	if (prop) {
		...
		I("%s: axis_map_y = %d\n", __func__, pdata->axis_map_y);
	} else
		I("%s: g_sensor_bma250,axis_map_y not found\n", __func__);
	...
	if (prop) {
		...
		pdata->negate_y = buf;
		I("%s: negate_y = %d\n", __func__, pdata->negate_y);
	} else
		I("%s: g_sensor_bma250,negate_y not found\n", __func__);

	prop = of_find_property(dt, "g_sensor_bma250,negate_z", NULL);
	if (prop) {
		of_property_read_u32(dt, "g_sensor_bma250,negate_z", &buf);
		pdata->negate_z = buf;
		I("%s: negate_z = %d\n", __func__, pdata->negate_z);
	} else
		I("%s: g_sensor_bma250,negate_z not found\n", __func__);
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0)
		goto exit_wrong_ID;
	D("%s: CHIP ID = 0x%02x\n", __func__, buffer[0]);
	if ((buffer[0] != 0x3) && (buffer[0] != 0xF9) && (buffer[0] != 0xFA)) {
	...
	numCount = 0;
	polling_limit = false;
	preserved_polling_interval = 200;
