/* drivers/htc_debug/mnemosyne/htc_mnemosyne.c
 *
...
struct mnemosyne_data *mnemosyne_phys = NULL;
EXPORT_SYMBOL(mnemosyne_phys);
struct mnemosyne_data *mnemosyne_base = NULL;
EXPORT_SYMBOL(mnemosyne_base);
phys_addr_t mnemosyne_size;
...
static void *mnemosyne_iomap(phys_addr_t phys, phys_addr_t size)
{
	if (!request_mem_region(phys, size, MNEMOSYNE_MODULE_NAME)) {
		pr_err("%s: request mem region (0x" HEX_FORMAT "@0x" HEX_FORMAT ") failed\n", MNEMOSYNE_MODULE_NAME, size, phys);
		return NULL;
	}
...
static int mnemosyne_setup(phys_addr_t phys, phys_addr_t size)
{
	...
	if (atomic_read(&mnemosync_is_init)) {
		if ((phys_addr_t)mnemosyne_phys != phys || (phys_addr_t)mnemosyne_size != size)
			WARN(1, "%s: init again with different old/new phys=0x%p/0x" HEX_FORMAT ", old/new size=0x" HEX_FORMAT "/0x" HEX_FORMAT "!\n",
	...
	if (!mnemosyne_base) {
		pr_err("%s: cannot map physical address 0x" HEX_FORMAT " size " DEC_FORMAT " to a valid virtual address.\n", MNEMOSYNE_MODULE_NAME,
				phys, size);
		return -ENOMEM;
	}

	if (sizeof(struct mnemosyne_data) > mnemosyne_size) {
		pr_err("%s: mnemosyne data size " DEC_FORMAT " exceeds reserved region size (" DEC_FORMAT").\n",
				MNEMOSYNE_MODULE_NAME, (phys_addr_t)sizeof(struct mnemosyne_data), mnemosyne_size);
		return -ENOMEM;
	}
	...
	pr_info("%s: size: " DEC_FORMAT " (" DEC_FORMAT " used)\n", MNEMOSYNE_MODULE_NAME, mnemosyne_size, (phys_addr_t)sizeof(struct mnemosyne_data));
	pr_info("%s: element info: " DEC_FORMAT "/%d/%d\n", MNEMOSYNE_MODULE_NAME,
			(phys_addr_t)sizeof(MNEMOSYNE_ELEMENT_TYPE),
...
static int rawdata_seq_show(struct seq_file *sfile, void *v)
{
	...
	for (i=0, rawdata+=meta[pos].index; i<meta[pos].num; i++) {
		...
		seq_printf(sfile, ": 0x" HEX_FORMAT " = " DEC_FORMAT "\n",
				rawdata[i], rawdata[i]);
	}
	return 0;
}
