...
static void himax_fwu_progress(int percentage);
#endif

#if defined(CONFIG_AK8789_HALLSENSOR)
static void himax_set_cover_func(unsigned int enable);

static DEFINE_MUTEX(himax_block_mutex);
static void himax_block_touch(struct himax_ts_data *ts, int enable)
{
       mutex_lock(&himax_block_mutex);
       ts->hall_block_touch_event = enable;
       mutex_unlock(&himax_block_mutex);
}

static void himax_block_touch_work_func(struct work_struct *dummy)
{
       struct himax_ts_data *ts = private_ts;
       himax_block_touch(ts, 0);
}
static DECLARE_DELAYED_WORK(himax_block_touch_work, himax_block_touch_work_func);

static void himax_handle_block_touch(struct himax_ts_data *ts, int enable)
{
       if (ts->hall_block_touch_event) {
               cancel_delayed_work(&himax_block_touch_work);
               himax_block_touch(ts, 0);
       }
       if (enable) {
               I("[HL] %s: %d\n", __func__, ts->hall_block_touch_time);
               schedule_delayed_work(&himax_block_touch_work, HZ*ts->hall_block_touch_time/1000);
               himax_block_touch(ts, 1);
       }
}
#endif
...
#ifdef CONFIG_OF
		I("%s, type28_selected, %p\n", __func__ ,(void *)type28_selected);
		if(config_load == false)
			{
...
static DEVICE_ATTR(register, (S_IWUSR|S_IRUGO),himax_register_show, himax_register_store);
	...
	static ssize_t himax_diag_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
	{
		...
		if (buf[0] == '1') {
			...
			diag_command ++;
			if(Is_2T3R){
				if (diag_command > 4) diag_command = 1;
			}else{
...
#ifdef CONFIG_TOUCHSCREEN_TOUCH_FW_UPDATE
				if (i == (3 * FileLength / 4))
					himax_fwu_progress(75);
				else if (i == (2 * FileLength / 4))
					himax_fwu_progress(45);
				else if (i == (1 * FileLength / 4))
					himax_fwu_progress(20);
#endif
			}
		}
		return 0;
	...
	static ssize_t himax_debug_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
	{
		...
		if (buf[0] == 't')
		{
			...
			if((sizeof(fileName) /sizeof(char)) > count-2)
...
static void doFirmwareUpdate(void *unused, async_cookie_t cookie)
{
	...
	int ret, FirmwareUpdate_Count=0;
	uint8_t buf[2] = {0};
	...
	if (ts->suspend_flag_b4_flash ^ suspend_status) {
		if (suspend_status) {
			...
			if (ret < 0)
			{
				E("%s: 1_I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
			}
			hr_msleep(30);
			...
			if (ret < 0)
			{
				E("%s: 2_I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
			}
			hr_msleep(30);
			...
			if (ret < 0)
			{
				E("%s: 3_I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
			}
			...
			if (ret < 0)
			{
				E("%s: 4_I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
			}
			hr_msleep(5);
...
static int updateFirmware(void *arg)
{
	...
	if (ts->fw == NULL) {
		...
		D("[FW] Request_firmware failed, ret = %d\n", ret);
		goto HMX_FW_REQUEST_FAILURE;
	}
...
int himax_touch_fw_update(struct firmware *fw)
{
	...
	himax_fwu_progress(0);
	
	snprintf(version, 10, "%02X", ts->vendor_fw_ver);
	I("[FW] version=%s\n", version);
	...
	ts->fw_data_start = (u8 *)(fw->data+tagLen);
	ts->fw_size = fw->size-tagLen;
...
int register_himax_touch_fw_update(void)
{
	...
	snprintf(himax_tp_notifier.fw_ver, sizeof(himax_tp_notifier.fw_ver), "0x%02X", ts->vendor_fw_ver);
...
#if defined(CONFIG_TOUCHSCREEN_PROXIMITY)
	sysfs_remove_file(android_touch_kobj, &dev_attr_ps_en.attr);
	sysfs_remove_file(android_touch_kobj, &dev_attr_ps_status.attr);
#endif
#if defined(CONFIG_AK8789_HALLSENSOR)
	if (private_ts->cover_support)
...
static int himax_parse_config(struct himax_ts_data *ts, struct himax_i2c_platform_data_config_type28 *pdata)
{
	...
	while ((pp = of_get_next_child(node, pp))) {
		...
		cfg_table[i].cover_support = of_property_read_bool(pp, "cover_support");
	...
	if (i <= cnt) {
		...
		private_ts->cover_support = cfg_table[i].cover_support;
		if (private_ts->cover_support)
			I(" DT-%s: cover_support", __func__);
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	} else if (support_htc_event_flag == 1) {
		pdata->support_htc_event = 1;
		I("%s support_htc_event = %d", __func__, pdata->support_htc_event);
	}
	...
	if (of_property_read_u32(dt, "update_feature", &data) == 0) {
		pdata->update_feature = data;
		I(" DT:update_feature=%d", pdata->update_feature);
	}
#if defined(CONFIG_AK8789_HALLSENSOR)
	if (of_property_read_u32(dt, "hall_block_touch_time", &data) == 0) {
		pdata->hall_block_touch_time = data;
		I(" DT:hall_block_touch_time=%d", pdata->hall_block_touch_time);
	}
#endif
...
static int himax8528_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->client = client;
	ts->dev = &client->dev;
	ts->dev->platform_data = &client->dev.platform_data;
	#if 0
	...
	if (pdata->gpio_3v3_en >= 0) {
		ret = gpio_request(pdata->gpio_3v3_en, "himax-3v3_en");
		if (ret < 0)
	...
	ts->update_feature = pdata->update_feature;
...
#endif
#if defined(CONFIG_TOUCHSCREEN_PROXIMITY)
	wake_lock_init(&ts->ts_wake_lock, WAKE_LOCK_SUSPEND, HIMAX8528_NAME);
#endif
	...
	if (private_ts->update_feature){
		I("[FW] process firmware update check.\n");
...
static void __init himax8528_init_async(void *unused, async_cookie_t cookie)
{
