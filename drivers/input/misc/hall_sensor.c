...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(pdata->gpio_att)) {
		ret = gpio_request(pdata->gpio_att, "hall_gpio_att_n");
		if (ret) {
			HL_ERR("Request IRQ failed, ret=%d, gpio=%d", ret, pdata->gpio_att);
			goto err_request_gpio_att_or_gpio_att_s_irq_failed;
		}
	}
	if (hl->att_used > 1){
		if (gpio_is_valid(pdata->gpio_att_s)){
			ret = gpio_request(pdata->gpio_att_s, "hall_gpio_att_s");
			if (ret) {
				HL_ERR("Request IRQ failed, ret=%d, gpio=%d", ret, pdata->gpio_att_s);
				goto err_request_gpio_att_or_gpio_att_s_irq_failed;
			}
		}
	}
