...
prev_sg_alloc_failed:
		if ((mqrq_cur->sg != host->cur_sg) && (mqrq_cur->sg != host->prev_sg))
			kfree(mqrq_cur->sg);
		mqrq_cur->sg = NULL;
cur_sg_alloc_failed:
		host->max_segs /= 2;
...
success:
 ...
 cleanup_queue:
	...
	if ((mqrq_prev->sg != host->cur_sg) && (mqrq_prev->sg != host->prev_sg))
		kfree(mqrq_prev->sg);
	mqrq_prev->sg = NULL;
