...
void mmc_stats(struct work_struct *work)
{
	...
	if (host->debug_flag & MMC_DEBUG_RANDOM_RW) {
	...
	if (host->debug_flag & MMC_DEBUG_FREE_SPACE) {
		struct file *file;
...
void mmc_start_debounce_phase1(struct mmc_host *host, unsigned long delay)
{
	pr_info("%s: %s\n", mmc_hostname(host), __func__);
	mmc_schedule_delayed_work(&host->debounce, delay);
}
...
void mmc_start_debounce_phase2(struct work_struct *work)
{
	struct mmc_host *host =
		container_of(work, struct mmc_host, debounce.work);
	pr_info("%s: %s\n", mmc_hostname(host), __func__);
...
void mmc_rescan(struct work_struct *work)
{
	...
	pr_info("%s: %s rescan_disable %d nonremovalbe %d rescan_entered %d\n", mmc_hostname(host), __func__,
		host->rescan_disable, (host->caps & MMC_CAP_NONREMOVABLE), host->rescan_entered);

	if (host->card && host->card->force_remove && !mmc_card_removed(host->card)) {
		pr_info("%s: %s remove card\n", mmc_hostname(host), __func__);
		mmc_bus_get(host);
					...
					unsigned long mode, void *unused)
{
	...
	pr_info("%s: %s mode %lu\n", mmc_hostname(host), __func__, mode);
	...
	case PM_POST_RESTORE:
		...
		mmc_detect_change(host, 0);
		#endif
		if (mmc_is_sd_host(host) && host->deferred_rescan) {
			host->rescan_disable = 0;
			host->deferred_rescan = 0;
			mmc_start_debounce_phase1(host, msecs_to_jiffies(50));
		}
		break;
