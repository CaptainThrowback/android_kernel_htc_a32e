...
inline struct htc_battery_cell *htc_battery_cell_find(int id_raw)
{
	...
	if(test_power_monitor || test_ftm_mode || test_download_mode) {
		pr_info("power_monitor(%d), ftm_mode(%d), download_mode(%d),"
				" load default batt params\n",
				test_power_monitor, test_ftm_mode, test_download_mode);
		pcell = &cells[0];
		return pcell;
	}
...
static int htc_batt_cell_probe(struct platform_device *pdev)
{
	...
	if (node) {
		...
		for_each_child_of_node(last_node, node) {
			...
			if(batt_cells != NULL)
				pr_info("[BATT] batt_cell:[%d] mapping with ID:[%d], range <%d ~ %d>, capacity:%d, voltage_max:%d, type:%d, model_name: %s\n",
				batt_cell_index,
				batt_cells[batt_cell_index].id,
				batt_cells[batt_cell_index].id_raw_min,
				batt_cells[batt_cell_index].id_raw_max,
				batt_cells[batt_cell_index].capacity,
				batt_cells[batt_cell_index].voltage_max,
				batt_cells[batt_cell_index].type,
				batt_cells[batt_cell_index].model_name);
