		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	snd_soc_jack_report_no_dapm(&__MBHC->headset_jack, __MBHC->hph_status, WCD_MBHC_JACK_MASK);
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (!(jack_type & SND_JACK_LINEOUT)) {
			...
			pr_info("%s: Skip reporting JACK_LINEOUT\n", __func__);
		}
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	if (!wrk_complete && plug_type == MBHC_PLUG_TYPE_HEADSET) {
		pr_info("%s: Headset already reported\n", __func__);
		goto enable_supply;
	}
...
static int wcd_mbhc_get_button_mask(u16 btn)
{
	...
	case 0:
		mask = SND_JACK_BTN_0;
		pr_info("[AUD][MBHC] press btn 0\n");
		break;
	case 1:
		mask = SND_JACK_BTN_1;
		pr_info("[AUD][MBHC] press btn 1\n");
		break;
	case 3:
		mask = SND_JACK_BTN_2;
		pr_info("[AUD][MBHC] press btn 2\n");
		break;
	case 7:
		mask = SND_JACK_BTN_3;
		pr_info("[AUD][MBHC] press btn 3\n");
		break;
	case 15:
		mask = SND_JACK_BTN_4;
		pr_info("[AUD][MBHC] press btn 4\n");
		break;
	default:
		break;
	}
